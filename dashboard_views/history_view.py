...
class HistoryView(QWidget):
    ...
    def __init__(
        self, config: DictaPilotConfig, store: TranscriptionStore, parent=None
    ):
    ...
    def _init_ui(self):
        ...
        self.page_label.setStyleSheet(
            f"color: {self.theme_manager.get_color('text-secondary')}; font-size: 14px;"
        )
    ...
    def _create_detail_panel(self) -> QWidget:
        ...
        # Empty state
        self.empty_detail = EmptyStateWidget(
    ...
    def _load_transcriptions(self):
        ...
        # Paginate
        total_pages = max(
            1, (len(filtered) + self.items_per_page - 1) // self.items_per_page
        )
        ...
        if not page_entries:
            ...
            if self.search_text or any(
                ...
                empty_widget = EmptyStateWidget(
                    "🔍",
                    "No transcriptions found matching your filters",
                    "Clear Filters",
                )
                empty_widget.action_clicked.connect(self._clear_filters)
            else:
                empty_widget = EmptyStateWidget(
                    "📝", "No transcriptions yet", "Start Dictating"
                )
    ...
    def _apply_filters(
        ...
        if self.search_text:
            ...
            filtered = [
                e
                for e in filtered
                if search_lower in (e.original_text or "").lower()
                or search_lower in (e.processed_text or "").lower()
            ]
        ...
        if self.filter_date_range and self.filter_date_range != "All Time":
            ...
            if cutoff:
                filtered = [
                    e for e in filtered if datetime.fromisoformat(e.timestamp) >= cutoff
                ]
    ...
    def _display_detail(self, entry: TranscriptionEntry):
        ...
        timestamp_label.setStyleSheet(
            f"color: {tokens['text-secondary']}; font-size: 13px;"
        )
        ...
        orig_label.setStyleSheet(
            f"color: {tokens['text-primary']}; font-size: 14px; margin-top: 8px;"
        )
        ...
        orig_text.setStyleSheet(
            f"color: {tokens['text-primary']}; font-size: 14px; padding: 8px; background-color: {tokens['bg-base']}; border-radius: 4px;"
        )
        ...
        meta_display.setStyleSheet(
            f"color: {tokens['text-secondary']}; font-size: 13px; padding: 8px; background-color: {tokens['bg-base']}; border-radius: 4px;"
        )
    ...
    def _export_csv(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Export Transcriptions", "transcriptions.csv", "CSV Files (*.csv)"
        )
        ...
        if filename:
            try:
                ...
                QMessageBox.information(
                    self,
                    "Export",
                    f"Exported {len(filtered)} transcriptions to {filename}",
                )
