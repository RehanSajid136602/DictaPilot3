...
from agent_orchestrator import AgentOrchestrator, AgentResponse
...
class WorkflowEngine:
    ...
    def _handle_spec_input(self, text: str, intent_result) -> Dict[str, Any]:
        ...
        if any(trigger in text.lower() for trigger in ["start new spec", "create spec", "new specification"]):
            return self._start_new_spec(text, intent_result)
        
        # If we're already in spec mode, update current spec
        if self.context.state in [WorkflowState.SPEC_CREATION, WorkflowState.SPEC_REFINEMENT]:
    ...
    def _update_current_spec(self, text: str) -> Dict[str, Any]:
        ...
        if not self.context.current_spec:
            return {
                "success": False,
                "error": "No active spec to update"
            }
    ...
    def _handle_command_input(self, text: str, intent_result) -> Dict[str, Any]:
        ...
        if "send to" in text_lower:
            agent_name = self._extract_agent_name(text)
            return self.send_to_agent(agent_name)
    ...
    def export_current_spec(self, format_style: str = "standard") -> Dict[str, Any]:
        ...
        if not self.context.current_spec:
            return {
                "success": False,
                "error": "No active spec to export"
            }
    ...
    def send_to_agent(self, agent_name: str, format_style: str = "standard") -> Dict[str, Any]:
        """Send current spec to an agent"""
        if not self.context.current_spec:
            return {
                "success": False,
                "error": "No active spec to send"
            }
        
        # Export spec
        content = self.spec_generator.export_spec(format_style)
        
        # Send to agent
        response = self.agent_orchestrator.send_to_agent(agent_name, content, format_style)
        
        if response.success:
            self.context.state = WorkflowState.AGENT_HANDOFF
            self.context.target_agent = agent_name
    ...
    def get_workflow_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "target_agent": self.context.target_agent,
    ...
    def _extract_title(self, text: str) -> str:
        ...
        match = re.search(r"(?:start new spec|create spec|new specification):\s*(.+?)(?:\.|$)", text, re.IGNORECASE)
        if match:
    ...
    def _extract_agent_name(self, text: str) -> str:
        ...
        if "cursor" in text_lower:
            return "cursor"
        elif "windsurf" in text_lower:
            return "windsurf"
        elif "cline" in text_lower:
            return "cline"
        elif "luna" in text_lower:
            return "luna"
        else:
