...
class BaseSettingsTab(QWidget):
    ...
    def add_heading(self, text: str):
        ...
        label.setStyleSheet(
            "font-size: 16px; font-weight: bold; color: #89b4fa; margin-top: 8px;"
        )
...
class GeneralSettingsTab(BaseSettingsTab):
    ...
    def _build_ui(self):
        ...
        self.mode_combo.setToolTip(
            "Dictation: standard text input\nAgent: structured coding tasks"
        )
        mode_form.addRow("Mode:", self.mode_combo)

        self.cli_auto_detect = QCheckBox("Auto-detect agent mode from speech")
        self.cli_auto_detect.setToolTip(
            "Automatically switch to agent mode when speech contains coding keywords"
        )
        mode_form.addRow("", self.cli_auto_detect)
        ...
        self.hotkey_combo.addItems(
            ["f9", "f10", "f11", "f12", "ctrl+shift+d", "alt+space"]
        )
        ...
        self.paste_mode.setToolTip(
            "Delta: only paste changed characters\nFull: select all and replace"
        )
        paste_form.addRow("Paste Mode:", self.paste_mode)

        self.paste_backend = QComboBox()
        self.paste_backend.addItems(
            ["auto", "wayland", "x11", "pynput", "xdotool", "keyboard"]
        )
        ...
        self.paste_policy.setToolTip(
            "Final only: paste after recording\nLive preview: show partial results"
        )
        ...
        self.voice_commands.setToolTip(
            "Process voice commands like 'delete that', 'new paragraph'"
        )
    ...
    def _load_config(self):
        ...
        self.cli_auto_detect.setChecked(self._config.cli_auto_detect)
    ...
    def get_config(self) -> DictaPilotConfig:
        ...
        self._config.cli_auto_detect = self.cli_auto_detect.isChecked()
...
class SmartEditingTab(BaseSettingsTab):
    ...
    def _build_ui(self):
        ...
        self.cleanup_strictness.setToolTip(
...
class ProfilesTab(BaseSettingsTab):
    ...
    def _build_ui(self):
        ...
        self.trigger_apps.setPlaceholderText(
            "One app name per line (e.g., code, vim, cursor)"
        )
...
class DictionaryTab(BaseSettingsTab):
    ...
    def _build_ui(self):
        ...
        # Info
        info = QLabel(
            "Dictionary entries are used to correct common transcription errors."
        )
...
class HistoryTab(BaseSettingsTab):
    ...
    def _refresh_list(self, entries=None):
        ...
        for entry in reversed(entries):
            preview = (
                entry.display_text[:60] + "..."
                if len(entry.display_text) > 60
                else entry.display_text
            )
    ...
    def _filter_by_date(self, period: str):
        ...
        entries = [e for e in self._store.get_all() if e.timestamp >= cutoff]
...
class AdvancedTab(BaseSettingsTab):
    ...
    def _build_ui(self):
        ...
        self.agent_webhook_url.setToolTip(
            "Webhook URL to receive formatted agent tasks"
        )
    ...
    def _load_config(self):
        ...
        self.reset_each_recording.setChecked(
            self._config.reset_transcript_each_recording
        )
    ...
    def get_config(self) -> DictaPilotConfig:
        ...
        self._config.reset_transcript_each_recording = (
            self.reset_each_recording.isChecked()
        )
...
def run_dashboard():
    ...
    # Launch floating window alongside legacy dashboard
    try:
        from app import GUIManager, Recorder, _setup_dictation_pipeline
        ...
        if hotkey_manager is None:
            print("Warning: Hotkey registration failed for floating window", file=sys.stderr)
    except Exception as e:
        print(f"Note: Floating window not available: {e}", file=sys.stderr)
    ...
    dashboard.setWindowState(
        dashboard.windowState() & ~Qt.WindowMinimized | Qt.WindowActive
    )
